...
namespace com.noorcon.rpg2e
{
	...
	public class GameMasterNetwork : NetworkBehaviour
	{
		public static GameMasterNetwork instance;
		...
		public GameLevelControllerNetwork LevelController;
		...
		void Awake()
		{
			...
			if (instance == null)
			{
				...
				instance.LevelController = new GameLevelControllerNetwork();
		...
		public void PlayerWeaponChanged(GameObject player, InventoryItem item)
		{
			Debug.Log(string.Format("Weapon changed to: {0}",
				instance.PlayerCharacterData.SelectedWeapon.ToString()));

			PlayerCharacter.WeaponType weapon = (PlayerCharacter.WeaponType)Enum.Parse(typeof(PlayerCharacter.WeaponType), item.Name);
			//player.GetComponent<BarbarianCharacterCustomization>().SetWeaponType(weapon);

			RpcUpdatePlayerWeapon(player, weapon);
		}

		[ClientRpc]
		public void RpcUpdatePlayerWeapon(GameObject player, PlayerCharacter.WeaponType weapon)
		{
			//PlayerCharacter.WeaponType weapon = (PlayerCharacter.WeaponType)Enum.Parse(typeof(PlayerCharacter.WeaponType), item.Name);
			player.GetComponent<BarbarianCharacterNetworkCustomization>().SetWeaponType(weapon);
		}
		...
		public void PlayerArmourChanged(InventoryItem item)
		{
			...
			RpcUpdatePlayerCharacter(item);

		}
		#endregion

		[ClientRpc]
		public void RpcUpdatePlayerCharacter(InventoryItem item)
		{
			switch (item.Type)
			{
				case InventoryItem.ItemType.Helmet:
					{
						instance.PlayerCharacterData.SelectedHelmet = (PlayerCharacter.HelmetType)Enum.Parse(typeof(PlayerCharacter.HelmetType),
							instance.PlayerCharacterData.SelectedArmour.Name);
						instance.PlayerCharacterGameObject.GetComponent<BarbarianCharacterNetworkCustomization>().SetHelmetType(instance.PlayerCharacterData.SelectedHelmet);
						break;
					}
				case InventoryItem.ItemType.Shield:
					{
						instance.PlayerCharacterData.SelectedShield = (PlayerCharacter.ShieldType)Enum.Parse(typeof(PlayerCharacter.ShieldType),
							instance.PlayerCharacterData.SelectedArmour.Name);
						instance.PlayerCharacterGameObject.GetComponent<BarbarianCharacterNetworkCustomization>().SetShieldType(instance.PlayerCharacterData.SelectedShield);
						break;
					}
				case InventoryItem.ItemType.ShoulderPad:
					{
						instance.PlayerCharacterData.SelectedShoulderPad = (PlayerCharacter.ShoulderPad)Enum.Parse(typeof(PlayerCharacter.ShoulderPad),
							instance.PlayerCharacterData.SelectedArmour.Name);
						instance.PlayerCharacterGameObject.GetComponent<BarbarianCharacterNetworkCustomization>().SetShoulderPad(instance.PlayerCharacterData.SelectedShoulderPad);
						break;
					}
